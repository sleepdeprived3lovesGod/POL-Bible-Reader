...
class BibleApp(tk.Tk):
    def __init__(self):
        ...
        self.default_translation = "pol.csv"
        ...
        self.current_translation = "pol.csv"
    ...
    def load_bible_data(self):
        ...
        try:
            ...
            self.books_data = self.bible_data[["Full Book Name", "Book Number"]].drop_duplicates()
            self.books_data = self.books_data.sort_values("Book Number")
            
            # Create dictionaries using Book Number and Full Book Name only
            self.number_to_book = dict(zip(self.books_data["Book Number"], self.books_data["Full Book Name"]))
            self.book_to_number = dict(zip(self.books_data["Full Book Name"], self.books_data["Book Number"]))
            
            # Remove references to book abbreviations
            self.book_abbrev_to_full = {}  # This can be empty as we're not using abbreviations
            self.book_full_to_abbrev = {}  # This can be empty as we're not using abbreviations
        except Exception as e:
    ...
    def load_last_read_verse(self):
        ...
        if self.read_verses:
            ...
            if not verse_data.empty:
                ...
                book_name = self.number_to_book[verse_data["Book Number"]]
                ...
                self.book_var.set(self.full_book_names[0])  # Set to first book
                self.update_chapters()
                self.chapter_var.set("1")
                self.update_verses()
                self.verse_var.set("1")
    ...
    def update_chapters(self):
        ...
        try:
            ...
            book_number = self.book_to_number[selected_book]
    ...
    def navigate(self, event=None):
        ...
        book_number = self.book_to_number[book]  # Use full book name directly
        chapter = int(chapter)
        verse = int(verse)
        ...
        self.verse_display.tag_remove("current", "1.0", "end")
        self.verse_display.tag_remove("read", "1.0", "end")
        ...
        for _, row in chapter_verses.iterrows():
            # Simplified verse reference format: just chapter:verse
            verse_text = f"{row['Chapter']}:{row['Verse']} {row['Text']}\n\n"
    ...
    def reset_chapter_history(self):
        ...
        if confirmation:
            book_number = self.book_to_number[self.book_var.get()]
