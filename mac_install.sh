...
if [ "$PYTHON_EXEC" == "python3.11" ]; then
    ...
    brew update
    brew install python-tk@3.11 python3.11-venv python3-pip "$PYTHON_DEV_PACKAGE"
fi
...
brew install libasound2-dev ffmpeg
...
if [ $? -ne 0 ]; then
    echo "ERROR: Failed to chown for brew python-tk."
    exit 1
fi

sudo chmod u+w /usr/local/share /usr/local/share/man /usr/local/share/man/man8
if [ $? -ne 0 ]; then
    echo "ERROR: Failed to chmod for brew python-tk."
    exit 1
fi

# Install python-tk/tkinter
echo "Installing python-tk/tkinter..."
brew install python-tk
if [ $? -ne 0 ]; then
    echo "ERROR: Failed to install python-tk/tkinter."
    exit 1
fi
